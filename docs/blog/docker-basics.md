...
## 포트 80의 두 가지 얼굴 — "HTTP는 위험한데 왜 80을 열지?"

- **엣지(Caddy)의 80**: 평문 서비스용이 아니라 ① 주소창에 도메인만 친 사용자를
  https로 301 리다이렉트 ② Let's Encrypt HTTP-01 챌린지(인증서 발급 검증) 통로.
  모든 HTTPS 사이트가 이렇게 80을 열어두고 안내만 한다
- **내부 컨테이너(admin-web)의 80**: `ports:` 선언이 없어 호스트 밖으로 노출 자체가
  안 된다. 경로: 사용자 ══HTTPS══> Caddy:443 (여기서 TLS 종료) ──평문──> admin-web:80.
  내부 구간은 같은 호스트 안 가상 네트워크라 패킷이 기계 밖으로 안 나감
- "HTTP 위험"의 정확한 의미는 **공용 인터넷 구간의 평문**. 엣지에서 암호화를 풀고
  내부는 평문으로 가는 **TLS 종료**가 업계 표준 (EKS도 ALB에서 종료 후 Pod까지 평문.
  내부까지 암호화하는 mTLS는 제로트러스트급 요구에서)
